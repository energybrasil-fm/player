import { stations } from './estacoes-3.3.js';
...
// VARIÁVEL PARA O MODO RELAXAR (ÁUDIO 3D)
...
function stopAndDiscardRecording() {
    ...
    const recordBtn = document.getElementById('recordBtn');
    if(recordBtn) {
        recordBtn.innerHTML = '<i class="bi bi-record-circle"></i> Gravar Áudio';
    }
}
...
window.toggleRelaxMode = function() {
    ...
    if (window.isRelaxModeOn) {
        ...
        btn.style.color = '#ef4444'; // Vermelho
        window.showGenericToast('Dolby Áudio', 'Dolby Atmos desativado.', 'bi bi-headphones', '#ef4444');
    }
    
    // Atualiza o gráfico de áudio se estiver reproduzindo
    if (visualizerActive) {
        routeAudioGraph();
    }
...
window.setSleepTimer = function(minutes) {
    ...
    sleepTimerTimeout = setTimeout(() => {
        if (isPlaying) {
            window.showGenericToast('Modo Soneca', 'A transmissão foi pausada.', 'bi-moon-stars-fill', '#3b82f6');
            toggleLivePlay(); 
        }
    }, ms);
...
// ==========================================
// EQUALIZADOR DSP 3D / AUTO CUE LEVELING
// ==========================================
...
// Nodes de Nivelamento (AGC + Limiter)
...
function initVisualizer() {
    if (!audioCtx) {
        ...
        try {
            ...
            // --- 1. Efeitos 3D Stereo (Spatializer / Haas Effect) ---
            ...
            // --- 2. Equalização Z-Bass & Z-Treble HD ---
            eqBass = audioCtx.createBiquadFilter();
            eqBass.type = 'lowshelf';
            eqBass.frequency.value = 85; 
            eqBass.gain.value = 4.5; 
            ...
            eqTreble.frequency.value = 10000;
            eqTreble.gain.value = 4.0; 

            // --- 3. AGC Leveler (Auto Cue de Volume) ---
            agcNode = audioCtx.createDynamicsCompressor();
            agcNode.threshold.value = -45;  
            agcNode.knee.value = 30;        
            agcNode.ratio.value = 8.0;      
            agcNode.attack.value = 0.005;   
            agcNode.release.value = 0.25;   
...
function routeAudioGraph() {
    ...
    // Desconecta tudo antes de reconstruir a rota para evitar duplicatas e "ecos" infinitos
    try {
        source.disconnect();
        splitter.disconnect();
        merger.disconnect();
    ...
    if (useEq) {
        // --- CONEXÃO DA CADEIA DSP COMPLETA ---
        source.connect(splitter);
        
        // Canal Left direto para o Merger (Pino 0)
        splitter.connect(merger, 0, 0);
        
        // Ativar ou Desativar apenas o 3D Delay Baseado na opção "Relaxar"
        if (window.isRelaxModeOn) {
            // Canal Right passa pelo Delay (Haas Effect 3D) -> Merger (Pino 1)
            splitter.connect(delayR, 1);
            delayR.connect(merger, 0, 1);
        } else {
            // Canal Right direto para o Merger (Sem 3D)
            splitter.connect(merger, 1, 1);
        }
        
        // A união estéreo entra no Equalizador
        merger.connect(eqBass);
        eqBass.connect(eqMid);
        eqMid.connect(eqTreble);
        
        // Entra no nivelador de volume (AGC + Makeup)
        eqTreble.connect(agcNode);
        agcNode.connect(makeupGain);
        
        // Entra no protetor (Limiter) e segue pro Visualizador e Caixas de Som
        makeupGain.connect(limiterNode);
        limiterNode.connect(analyser); 
        analyser.connect(audioCtx.destination);
    } else {
        // --- BYPASS --- 
        // Apenas o visualizador circular animado será alimentado pelo áudio cru
...
// ==========================================
// SISTEMA DE REPRODUÇÃO INSTANTÂNEA E CACHE (S/ FADE BUGADO)
// ==========================================
...
/* Função reconstruída para remover sistema instável de alteração de volume e focar na robustez do Play/Pause direto. */
    ...
    if (action === 'in') {
        ...
        if (playPromise !== undefined) {
            playPromise.then(() => {
                ...
                isSwitchingQuality = false; 
                // Se não for interrupção normal e estiver tocando o rádio principal, tentar reconectar
                if (e.name !== 'AbortError' && player === audioPlayer) { 
        ...
        // Pause direto anula totalmente o uso bugado do volume do iOS para parar fluxos web audio
        player.pause();
        if (callback) callback();
    }
}
...
function startMainStream() {
    ...
    // Conecta na borda ao vivo garantindo que não carregue áudio velho armazenado no buffer do src atual
...
function toggleLivePlay() {
    ...
    if (!isPlaying) {
        ...
        if (isPlayingIntro) {
            ...
            fadeAudio(audioPlayer, 'out', () => {
                // Remove source attribute and reload so that internal buffer resets and 
                // graph disconnection is forced cleanly
