import { stations } from './estacoes-2.9.js';
...
// ==========================================
// GRAVAÇÃO DE ÁUDIO DIRETA COM BLOQUEIO
// ==========================================
...
window.toggleRecording = async function() {
    ...
    if (station.record === false || String(station.record).toLowerCase() === "false") {
        window.showGenericToast('Gravação Bloqueada', 'Esta estação não permite a gravação da transmissão.', 'bi-slash-circle', '#f43f5e');
...
// ==========================================
// EQUALIZADOR DE ÁUDIO HD (Stereo Tool DSP)
// ==========================================
let audioCtx, analyser, source;
let visualizerActive = false;

// Nodes do DSP
let zBass, punchBass, presence, zTreble, agcNode, makeupGain, limiterNode;
...
function initVisualizer() {
    if (!audioCtx) {
        ...
        try {
            ...
            // 1. AGC (Leveler) - Traz as músicas baixas para cima suavemente
            agcNode = audioCtx.createDynamicsCompressor();
            agcNode.threshold.value = -35;
            agcNode.knee.value = 30;
            agcNode.ratio.value = 2.5;
            agcNode.attack.value = 0.1; // Lento o suficiente para não afetar batidas
            agcNode.release.value = 0.5; // Solta rápido mas sem pumping
            
            // 2. Z-Bass (Subgraves imersivos profundos)
            zBass = audioCtx.createBiquadFilter();
            zBass.type = 'lowshelf';
            zBass.frequency.value = 55;
            zBass.gain.value = 3.5;
            
            // 3. Punch Bass (Batida seca estilo JBL, joga o som para fora)
            punchBass = audioCtx.createBiquadFilter();
            punchBass.type = 'peaking';
            punchBass.frequency.value = 110;
            punchBass.Q.value = 1.2;
            punchBass.gain.value = 2.0;

            // 4. Clarity/Presence (Tira abafado sem distorcer voz - HD Stereo)
            ...
            // 5. Z-Treble (Agudos cristalinos, sensação de áudio Lossless)
            zTreble = audioCtx.createBiquadFilter();
            zTreble.type = 'highshelf';
            zTreble.frequency.value = 11000;
            zTreble.gain.value = 4.0;

            // 6. Makeup Gain Elevado (Garante som forte para qualquer dispositivo)
            makeupGain = audioCtx.createGain();
            makeupGain.gain.value = 2.2; 

            // 7. Brickwall Limiter (Corta distorções finais com attack instantâneo)
            limiterNode = audioCtx.createDynamicsCompressor();
            limiterNode.threshold.value = -1.5;
            limiterNode.knee.value = 0.0;
            limiterNode.ratio.value = 20.0; // Padrão limiter
            limiterNode.attack.value = 0.002; // Ação imediata nos picos
...
function routeAudioGraph() {
    ...
    // Desconecta tudo antes de reconstruir a rota
    try {
        source.disconnect();
        agcNode.disconnect();
        zBass.disconnect();
        punchBass.disconnect();
        presence.disconnect();
        zTreble.disconnect();
    ...
    if (useEq) {
        // Conexão completa com equalizador HD e Leveling Automático
        source.connect(agcNode);
        agcNode.connect(zBass);
        zBass.connect(punchBass);
        punchBass.connect(presence);
        presence.connect(zTreble);
        zTreble.connect(makeupGain);
        makeupGain.connect(limiterNode);
        limiterNode.connect(analyser);
        analyser.connect(audioCtx.destination);
    } else {
        // Conexão direta mantendo apenas o visualizador ativo (bypass no equalizador)
...
function drawVisualizer() {
    ...
    for (let l = 0; l < layers; l++) {
        ...
        ctx.strokeStyle = `rgba(${r}, ${g}, ${b}, ${1 - (l * 0.2)})`; 
        ctx.lineWidth = 3 - l; 
        ctx.shadowBlur = 15; 
        ...
        for (let i = 0; i < points; i++) {
            let dataIndex = Math.floor((i / points) * (dataArray.length / 2));
            let rawValue = dataArray[dataIndex] || 0;
            
            let prevIndex = Math.max(0, dataIndex - 1);
            let nextIndex = Math.min(dataArray.length - 1, dataIndex + 1);
            let smoothedValue = (dataArray[prevIndex] + rawValue + dataArray[nextIndex]) / 3;

            let mappedValue = (smoothedValue / 255) * (35 - (l * 5)); 
            
            let time = Date.now() * 0.002;
            let waveOffset = Math.sin((i / points) * Math.PI * 6 + time + l) * (4 + l * 2);

            let radius = baseRadius + mappedValue + waveOffset;
        ...
        let startXc = (coords[0].x + coords[points - 1].x) / 2;
        let startYc = (coords[0].y + coords[points - 1].y) / 2;
        ctx.moveTo(startXc, startYc);
        
        for (let i = 0; i < points; i++) {
            let p1 = coords[i];
            let p2 = coords[(i + 1) % points];
...
// ==========================================
// SISTEMA DE FADE, CACHE E LÓGICA DO INTRO
// ==========================================
...
function setBufferingState(isBuffering) {
    ...
    if (isBuffering && !isOfflineStatus && !isPlayingIntro) {
        els.forEach(el => {
            if (el.t) {
...
if(audioPlayer) {
    audioPlayer.addEventListener('waiting', () => { 
        if (isPlaying && !isSwitchingQuality && !isPlayingIntro) {
    ...
    audioPlayer.addEventListener('error', () => { 
        if (isPlaying && !isSwitchingQuality && !isPlayingIntro) { clearTimeout(offlineTimeout); setBufferingState(false); setOfflineState(true); handleReconnect(); }
    });

    audioPlayer.addEventListener('stalled', () => { 
        if (isPlaying && audioPlayer.readyState === 0 && !isSwitchingQuality && !isPlayingIntro) { 
            setLoadingState(true); setBufferingState(true); clearTimeout(offlineTimeout);
            offlineTimeout = setTimeout(() => { setBufferingState(false); setOfflineState(true); handleReconnect(); }, 20000); 
        }
    });
    ...
    audioPlayer.addEventListener('pause', () => {
        if (isPlaying && !activePreviewId && !isSwitchingQuality && !isPlayingIntro) {
            let retryInterval = setInterval(() => {
                if (!isPlaying || activePreviewId || isSwitchingQuality || isPlayingIntro) { clearInterval(retryInterval); return; } 
...
window.addEventListener('offline', () => { if (isPlaying && !isPlayingIntro) { clearTimeout(offlineTimeout); setBufferingState(false); setOfflineState(true); handleReconnect(); } });
window.addEventListener('online', () => { if (isOfflineStatus && isPlaying && !isPlayingIntro) handleReconnect(); });
        ...
        if (playerProgressBar && bufferEnd > 0 && isFinite(bufferEnd)) {
            ...
            if (bufferEnd - current > 4) {
                ...
                if (dvrTimeDisplay) {
                    ...
                    dvrTimeDisplay.textContent = `-${formatTimeString(diff)}`;
                }
            } else {
                if(playerLiveBadge) {
                ...
                if (dvrTimeDisplay) dvrTimeDisplay.textContent = formatTimeString(totalListenedTime);
            }
        }
    });
}
...
// ==========================================
// SISTEMA DE QUALIDADE DE ÁUDIO COM ADAPTAÇÃO DINÂMICA
// ==========================================
...
function evaluateAutoQuality() {
    ...
    // Evita trocas excessivas caso a rede fique oscilando rapidamente
    if (autoQualityCooldown || isSwitchingQuality) return;
    ...
    if (navigator.connection) {
        ...
        // Lógica rigorosa de adaptação de velocidade
        if (saveData) {
            targetQuality = 'low';
        } else if (type === 'slow-2g' || type === '2g' || downlink < 0.6 || rtt > 450) {
            targetQuality = 'low';
        } else if (type === '3g' || (downlink >= 0.6 && downlink < 2.5) || rtt > 150) {
            targetQuality = 'mid';
    ...
    if (activeQualityLevel !== targetQuality) {
        let qualityName = targetQuality === 'high' ? 'Excelente' : targetQuality === 'mid' ? 'Normal' : 'Econômica';
        let iconType = targetQuality === 'high' ? 'bi-wifi' : targetQuality === 'mid' ? 'bi-reception-3' : 'bi-reception-1';
        
        // Notifica o usuário sobre a mudança dinâmica
        if (isPlaying) {
            window.showGenericToast('Rede Alterada', `Ajustando qualidade para <strong>${qualityName}</strong> para evitar travamentos.`, iconType, '#3b82f6');
        }
        
        applyQualityStream(targetQuality);
        
        // Cooldown de 20 segundos para evitar que a rádio fique "picotando" se a rede oscilar no limite
        autoQualityCooldown = true;
        setTimeout(() => { autoQualityCooldown = false; }, 20000);
    } else {
        updateQualityBadges();
    }
}
...
function applyQualityStream(quality) {
    ...
    if (oldQuality === quality && audioPlayer && audioPlayer.src !== "") return;
    ...
    } else { if(audioPlayer) audioPlayer.src = getCurrentStreamUrl(); }
}
...
// ==========================================
// VOLUME E PLAYBACK COM LOGICA DE INTRO
// ==========================================
...
function toggleMute(e) {
    ...
    if(introAudio) introAudio.volume = globalVolume;
...
function startMainStream() {
    ...
    if(audioPlayer && !audioPlayer.src) audioPlayer.src = getCurrentStreamUrl(); 
...
function playIntro(introUrl) {
    ...
    let introTimeout = setTimeout(() => {
        if (isPlayingIntro) {
            console.warn("Intro timeout, starting main stream");
            isPlayingIntro = false;
            startMainStream();
        }
    }, 8000); 
    ...
    introAudio.play().then(() => {
        ...
        if (currentTrackMetadata && currentTrackMetadata.title) {
            updateUIText(currentTrackMetadata);
        }
    }).catch(e => {
        clearTimeout(introTimeout);
        console.warn("Intro failed to play:", e);
        isPlayingIntro = false;
        startMainStream();
    });

    introAudio.onended = () => { if(isPlayingIntro) { isPlayingIntro = false; startMainStream(); } };
    introAudio.onerror = () => { if(isPlayingIntro) { isPlayingIntro = false; startMainStream(); } };
}
...
window.changeStation = function(index) {
    ...
    if (isPlaying) {
        ...
        if (isPlayingIntro) {
            ...
            fadeAudio(audioPlayer, 'out', proceedWithNewStation);
        }
    } else { 
        if(audioPlayer) {
            audioPlayer.removeAttribute('src');
...
if (homeOpenStationsBtn) {
    homeOpenStationsBtn.addEventListener('click', () => { 
        renderStationsModal(); 
        const stModal = document.getElementById('stationsModal');
...
function updateUIText(metadata) {
    ...
    if ((isOfflineStatus && !isPlayingIntro) || (isBufferingStatus && !isPlayingIntro)) {
...
async function renderHistory(historyArray, fallbackArt) {
    ...
    // Pega o título da música que está tocando agora
    ...
    for (const item of historyArray) {
        ...
        // Verifica se a música não é a que está tocando agora para evitar que apareça imediatamente
...
// LOGICA DE OCULTAÇÃO E BLOQUEIOS QUANDO HOUVER APENAS 1 ESTAÇÃO
if (stations.length <= 1) {
    // Esconde o botão de Estações da HOME e centraliza o play amarelo
    const homeOpenStationsBtn = document.getElementById('homeOpenStationsBtn');
    ...
    // Bloqueia e aplica opacidade baixa no botão "Estações" de dentro do player
