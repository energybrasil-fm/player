        import { stations } from './estacoes-1.3.js';
        import { updateIntervalTime, isValidTrack, fetchItunesData, fetchStationData, fetchLyrics } from './api_config-1.2.js';
...
function renderQualityMenu() {
    ...
    validQualities.forEach(q => {
        ...
        const isManualSelected = (currentQualityMode === q.id) || (!hasMultipleStreams && isActiveStream);
...
window.changeStation = function(index) {
    ...
    if (isPlaying) {
        ...
        // Se a estação mudou pela lista e o usuário está na Home Screen, o botão "Ouvir" balança chamando atenção
        const hBtn = document.getElementById('homePlayBtn');
        if (hBtn && !isPlaying) {
            hBtn.classList.remove('attention-shake');
            void hBtn.offsetWidth; 
            hBtn.classList.add('attention-shake');
        }
    }
}
...
if(openStationsBtn) openStationsBtn.addEventListener('click', openStationsAction);
if(document.getElementById('homeStationsBtn')) document.getElementById('homeStationsBtn').addEventListener('click', openStationsAction);
...
// ==========================================
// INICIALIZAÇÃO E CHECK DE ESTAÇÃO ÚNICA
// ==========================================
...
// ESCONDER BOTÃO DE ESTAÇÕES SE TIVER APENAS 1
if (stations.length <= 1) {
    const btnEstacoes = document.getElementById('openStationsBtn'); 
    if (btnEstacoes) { 
        btnEstacoes.style.opacity = '0.4'; 
        btnEstacoes.style.pointerEvents = 'none'; 
        btnEstacoes.style.cursor = 'not-allowed'; 
    }
    
    const homeBtnEstacoes = document.getElementById('homeStationsBtn'); 
    if (homeBtnEstacoes) { homeBtnEstacoes.style.display = 'none'; }
