import { stations } from './estacoes-3.1.js';
...
// ==========================================
// EQUALIZADOR DSP / AGC PROFISSIONAL
// ==========================================
let audioCtx, analyser, source;
let visualizerActive = false;
let subFilter, bassFilter, punchFilter, trebleFilter, agcNode, makeupGain, limiterNode;
...
function initVisualizer() {
    if (!audioCtx) {
        ...
        try {
            ...
            // 1. Sub-bass cut (Protege caixas JBL/Boombox de distorcer com "lama" subsônica)
            subFilter = audioCtx.createBiquadFilter();
            subFilter.type = 'highpass';
            subFilter.frequency.value = 25; 
            subFilter.Q.value = 0.7;

            // 2. Z-Bass (Grave profundo, espalhado para fora e limpo)
            bassFilter = audioCtx.createBiquadFilter();
            bassFilter.type = 'lowshelf';
            bassFilter.frequency.value = 85; 
            bassFilter.gain.value = 4.5; 

            punchFilter = audioCtx.createBiquadFilter();
            punchFilter.type = 'peaking';
            punchFilter.frequency.value = 55;
            punchFilter.Q.value = 1.2;
            punchFilter.gain.value = 2.5;

            // 3. Z-Treble (Agudos cristalinos HD, sensação de imersão/Stereo Espalhado)
            trebleFilter = audioCtx.createBiquadFilter();
            trebleFilter.type = 'highshelf';
            trebleFilter.frequency.value = 9500;
            trebleFilter.gain.value = 3.5; 

            // 4. AGC - Automatic Gain Control (Nivelador de volume anti-pumping)
            agcNode = audioCtx.createDynamicsCompressor();
            agcNode.threshold.value = -35;  // Pega músicas baixinhas lá no fundo
            agcNode.knee.value = 40;        // Transição macia, sem estragos
            agcNode.ratio.value = 2.5;      // Nível de compressão moderado (não estraga a dinâmica)
            agcNode.attack.value = 0.5;     // Attack Lento: ignora os graves e evita o "Pumping"
            agcNode.release.value = 1.0;    // Release suave e natural

            // 5. Gain de compensação do AGC
            makeupGain = audioCtx.createGain();
            makeupGain.gain.value = 2.8;    // Joga o volume de tudo para o padrão HD 

            // 6. Limiter Peak (Garante ZERO distorção em caixas de som pesadas e fones)
            limiterNode = audioCtx.createDynamicsCompressor();
            limiterNode.threshold.value = -1.0; 
            limiterNode.knee.value = 0.0; 
            limiterNode.ratio.value = 20.0; 
            limiterNode.attack.value = 0.002; // Attack ultra rápido
...
function routeAudioGraph() {
    ...
    // Desconecta tudo antes de reconstruir a rota para evitar duplicatas
    try {
        source.disconnect();
        subFilter.disconnect();
        bassFilter.disconnect();
        punchFilter.disconnect();
    ...
    if (useEq) {
        // Rota Completa com Imersão, AGC Nivelado, Zero Distorção e Zero Pumping
        source.connect(subFilter);
        subFilter.connect(bassFilter);
        bassFilter.connect(punchFilter);
        punchFilter.connect(trebleFilter);
        trebleFilter.connect(agcNode);
        agcNode.connect(makeupGain);
        makeupGain.connect(limiterNode);
        limiterNode.connect(analyser); // O Analisador pega o áudio já tratado para pulsar melhor
        analyser.connect(audioCtx.destination);
    } else {
        // Rota Direta (Bypass Total: Sem Eq e Sem AGC, apenas o visualizador de tela funciona)
