...
function fetch100HitzJSONP(url) {
    return new Promise((resolve, reject) => {
        ...
        window[callbackName] = function(data) {
            ...
            delete window[callbackName];
            if (document.body.contains(script)) document.body.removeChild(script);
            if (data && data.contents) { resolve(data.contents); } else { reject(new Error("Resposta vazia")); }
        };
    ...
    try {
        ...
        // ==============================================================
            ...
            if (station.type === 'azuracast') {
                ...
                if (rootData && rootData["current-track"]) { const track = rootData["current-track"]; songObj = { title: track.title || '', artist: track.artist || '', art: track.art || station.defaultArt }; }
