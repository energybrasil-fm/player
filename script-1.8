import { stations } from './estacoes-1.7.js';
...
window.toggleRecording = async function() {
    ...
    // VERIFICAÇÃO DE BLOQUEIO DE GRAVAÇÃO DA ESTAÇÃO
    if (station.record === false || String(station.record).toLowerCase() === "false") {
        window.showGenericToast('Gravação Bloqueada', 'Esta estação não permite a gravação da transmissão.', 'bi-ban', '#f43f5e');
...
function setOfflineState(isOffline) {
    ...
    if (isOffline) {
        ...
        if (!isBufferingStatus && !isPlayingIntro && currentTrackMetadata && currentTrackMetadata.title) updateUIText(currentTrackMetadata);
    }
}
...
function playIntro(introUrl) {
    ...
    introAudio.play().then(() => {
        ...
        if(titleEl) { titleEl.textContent = "Apresentação..."; applyMarquee(titleEl); }
        if(artistEl) { artistEl.innerHTML = `<span>Aguarde a transmissão...</span>`; }
...
function toggleLivePlay() {
    ...
    if (!isPlaying) {
        ...
        if (station.intro && station.intro.trim() !== "") {
...
function updateUIText(metadata) {
    ...
    if (isOfflineStatus || isBufferingStatus || isPlayingIntro) {
