...
// ==========================================
function fetchWithJQueryAjaxText(url) {
...
function fetchWithJQueryJSONP(url) {
    return new Promise((resolve, reject) => {
        const doAjax = () => {
            window.jQuery.ajax({
                url: url,
...
// ==========================================
// SISTEMA ANTI-TRAVAMENTO E PARSER JSON
// ==========================================
const fetchCache = {};
...
async function safeFetchJson(url) {
    ...
    const fetchPromise = (async () => {
        try {
            // Tenta sempre usar o Fetch direto primeiro (Mais Rápido e Nativo)
            const response = await fetch(url, { mode: 'cors' });
            if (response.ok) { return await parseJsonResponse(response); }
        } catch (err) {
            console.warn(`Tentativa nativa falhou, recorrendo ao proxy para: ${url}`);
        }

        // Se o CORS bloquear a requisição direta, usa os proxies como Fallback
        const proxies = [
...
export async function fetchItunesData(title, artist) {
    ...
    const fetchPromise = (async () => {
        try {
            ...
            const res = await fetch(`https://itunes.apple.com/search?term=${cleanQuery}&entity=song&limit=5`, { mode: 'cors' });
            ...
            const invalidTerms = [
                ...
                'instrumental', 'live', 'various artists', 'vários artistas', 'mashup'
            ].map(t => t.toLowerCase());
...
export async function fetchStationData(station) {
    ...
    try {
        ...
        // Auto-Discovery: Extrai o Domínio+Porta da stream ativa para Shoutcast/Icecast se a URL da API for vazia
        let baseUrl = "";
        if (station.streams) {
        ...
        // ==============================================================
        // 100HITZ / GOTRADIO (Via jQuery AJAX Texto Puro - Direto sem Proxy)
        // ==============================================================
        ...
        // ==============================================================
        // SHOUTCAST (Auto-Discovery via JSONP e 7.html sem Proxy)
        // ==============================================================
        else if (station.type === 'shoutcast') {
            const targetBase = (station.api && station.api.trim() !== "") ? station.api.replace(/\/stats.*|\/7\.html.*/, '').replace(/\/$/, '') : baseUrl;
            let sid = 1; 
            
            try {
                // Tenta V2 nativo JSONP (Bypassa CORS naturalmente)
                ...
                // Se o V2 JSONP falhar, tenta o V1 (7.html) e History (played.html) via jQuery Texto
                try {
        ...
        // ==============================================================
        // ICECAST (Auto-Discovery status-json.xsl)
        // ==============================================================
        else if (station.type === 'icecast') {
            const targetUrl = (station.api && station.api.trim() !== "") ? station.api : baseUrl + "/status-json.xsl";
            const icecastTimestampedUrl = targetUrl + (targetUrl.includes('?') ? '&' : '?') + "t=" + new Date().getTime();
        ...
        // ==============================================================
        // DEMAIS APIS JSON (Azuracast, JamFM, NRJ, EHR, FFH, etc)
        // ==============================================================
        else {
            ...
            if (station.type === 'azuracast') {
                ...
                listeners = 0; 
                let tracks = [];
                if (rootData.EHR && Array.isArray(rootData.EHR)) {
                    tracks = rootData.EHR;
                ...
                if (tracks.length > 0) {
                    ...
                    if (station.historyApi && station.historyApi !== "" && station.historyApi !== station.api) {
                        try {
                            ...
                            let histTracks = [];
                            if (histData.EHR && Array.isArray(histData.EHR)) {
                                histTracks = histData.EHR;
            ...
            // ==============================================================
            // FFH / Planet Radio / Harmony FM (Descoberta Inteligente JSON Array)
            // ==============================================================
        ...
        // ==============================================================
        // LIMPEZA FINAL DO HISTÓRICO DE MÚSICAS
        // ==============================================================
        if (historyObj && historyObj.length > 0) {
            ...
            for (const item of historyObj) {
                ...
                if (isValidTrack(tTitle, tArtist)) {
                    ...
                    if (currentTrackString !== lastTrackString) { 
                        cleanHistory.push(item); 
                        lastTrackString = currentTrackString; 
                    }
                }
            }
