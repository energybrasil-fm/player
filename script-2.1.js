import { stations } from './estacoes-1.8.js';
        import { updateIntervalTime, isValidTrack, fetchItunesData, fetchStationData, fetchLyrics } from './api_config-1.7.js';
...
function playIntro(introUrl) {
    ...
    // Injeta silenciosamente o stream principal para gerar cache background
    ...
    introAudio.play().then(() => {
        ...
        // Garante que o texto oficial da API apareça durante a intro
...
function toggleLivePlay() {
    ...
    if (!isPlaying) {
        ...
        // Se a estação tiver Intro e ela ainda NÃO tocou nesta sessão de play
...
window.changeStation = function(index) {
    ...
    // Reseta a intro para permitir que toque quando entrar na nova rádio
    introPlayedForCurrentStation = false; 
    
    const station = stations[index];

    // ATUALIZAÇÃO DO BOTÃO DE GRAVAÇÃO (BLOQUEIO POR ESTAÇÃO)
    const recordBtn = document.getElementById('recordBtn');
    if (recordBtn) {
        if (station.record === false || String(station.record).toLowerCase() === "false") {
...
function updateUIText(metadata) {
    ...
    // Mantém a visualização dos dados se a intro estiver rodando.
